...
if(this.selectedFile == null){
  ...
  this.messageService.sendName(this.ownername,this.chatroomname,this.singlemessage,this.selectedFile.item(0).name);
    ...
    //     if(JSON.parse(this.mymessages[i]).destination != this.chatroomname){
    //             this.mymessages.splice(this.mymessages.indexOf(this.mymessages[i]), 1)
    //           }
    //   }
      })
    this.posttoServer();
...
deleteAllProfiles(){
  ...
  this.http.delete<any>("http://localhost:8080/deleteallProfiles").toPromise()
    .then((data) => {this.chatRoom_data = JSON.parse(JSON.stringify(data[0]));
